...
public class TicketMapRenderer {

    public void generateAndDisplay(World world, Location baseLocation) {
        int tileSize = 128;
        int tilesX = 8;
        int tilesY = 8;
        int totalWidth = tileSize * tilesX;   // 1024
        int totalHeight = tileSize * tilesY;  // 1024

        BufferedImage image;
        try {
            InputStream stream = getClass().getResourceAsStream("/maps/templates/testMap/map.png");
            if (stream == null) {
                return;
            }
